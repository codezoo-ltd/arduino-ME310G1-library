...
void setup() {
  Serial.begin(115200);
  MDMSerial.begin(115200, SERIAL_8N1, 37, 38);  //RX2:37, TX2:38
  ...
  // issue command AT+cpin? in read mode, check
  ...
  if (rc == ME310::RETURN_VALID) {
    ...
    if (resp != NULL) {
	  // read response in 2 array position
      if (strcmp(resp, "OK") == 0) 
      {
        ...
        if (rc == ME310::RETURN_VALID) {
		  // issue command AT+CGDCONT=? (read mode)
          myME310.read_define_pdp_context(); 
          Serial.print("pdp context read :");
		  // print second line of modem answer
		  ...
		  // issue command AT#SGACT=cid,state and
...
void loop() {
  ...
  if (rc == ME310::RETURN_VALID) {
    delay(2000);
	// issue command AT#FTPOPEN=ftp address and port, ftp
    // user and ftp password and wait for answer or timeout
    ...
    if (rc == ME310::RETURN_VALID) {
      ...
      // issue command AT#FTPCWD=path directory and wait for answer or timeout
      rc = myME310.ftp_change_working_directory("/CristinaDe", ME310::TOUT_10SEC); 
	  Serial.println(myME310.return_string(rc)); // returns a string with return_t codes
      if (rc == ME310::RETURN_VALID) {
        Serial.print("Ftp working directory: ");
		// issue command AT#FTPPWD and wait for answer or timeout
        myME310.ftp_print_working_directory(ME310::TOUT_10SEC); 
        Serial.println(myME310.buffer_cstr(1));
		...
		// issue command AT#FTPPUT=filename,connMode and
        // wait for answer or timeout
        rc = myME310.ftp_put("test.txt", 1, ME310::TOUT_10SEC);
        Serial.println((String) "Ftp put result: " + myME310.return_string(rc));
        if (rc == ME310::RETURN_VALID) {
		  // issue command AT#FTPAPPEXT=dataSize, 
		  // data and wait for answer or timeout
          rc = myME310.ftp_append_extended((int)sizeof(data), (char *)data, 1); 
          ...
          if (rc == ME310::RETURN_VALID) {
			...
			// issue command AT#FTPGETPKT=file name and 
			// wait for answer or timeout
            rc = myME310.ftp_get_command_mode("test.txt", 0, ME310::TOUT_20SEC); 
            Serial.println((String) "Ftp get in command mode result: " + myME310.return_string(rc));
            if (rc == ME310::RETURN_VALID) {
			  // issue command AT#FTPRECV=block size and
              // wait for answer or timeout
              rc = myME310.ftp_receive_data_command_mode((int)sizeof(data), ME310::TOUT_10SEC); 
              if (rc == ME310::RETURN_VALID) {
		  ...
		  // issue command AT#FTPDELE=file name and wait
          // for answer or timeout
          rc = myME310.ftp_delete("test.txt", ME310::TOUT_30SEC); 
          if (rc == ME310::RETURN_VALID) {
