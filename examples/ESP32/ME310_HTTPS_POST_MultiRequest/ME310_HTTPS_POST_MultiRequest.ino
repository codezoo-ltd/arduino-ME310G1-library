...
void setup() {
  ...
  MODEM_SERIAL.begin(115200, SERIAL_8N1, 16, 17);  //RX2:16, TX2:17
  ...
  DEBUG_SERIAL.println("======================================");
  DEBUG_SERIAL.println(" ME310G1 HTTPS POST Request Test      ");
  DEBUG_SERIAL.println("======================================");
  ...
  // ---------------------------------------------------------
  // [STEP 3] 1st HTTPS POST Request: Raw String (Direct Call)
  // ---------------------------------------------------------
  DEBUG_SERIAL.println("\n[HTTPS] 2. Sending 1st HTTPS POST Request (Raw String)...");
  
  const char* rawPayload = "temp=25.3";
  int rawPayloadLen = strlen(rawPayload);
  
  // Send data using the direct public function for requests without parameters
  rc = myME310.send_http_send_without_params(0, 0, "/post", rawPayloadLen, (char*)rawPayload, ME310::TOUT_5SEC);
  
  if (rc == ME310::RETURN_VALID) {
    DEBUG_SERIAL.println("-> 1st SND Command Accepted & Data Sent. Waiting for Secure Server...");
    catchHttpRingAndReceive(0, 15000); 
  } else {
    DEBUG_SERIAL.println("-> [ERROR] 1st HTTPS SND Command Failed.");
  }

  delay(3000); // Give the modem a brief rest before the next secure transaction

  // ---------------------------------------------------------
  // [STEP 4] 2nd HTTPS POST Request: JSON Payload (With content-type)
  // ---------------------------------------------------------
  DEBUG_SERIAL.println("\n[HTTPS] 3. Sending 2nd HTTPS POST Request (JSON Payload)...");
  
  const char* jsonPayload = "{\"temp\":25.3}";
  int jsonPayloadLen = strlen(jsonPayload);
  
  // Prevent comma formatting issues in internal library snprintf by injecting a harmless extra header
  rc = myME310.send_http_send(0, 0, "/post", jsonPayloadLen, (char*)jsonPayload, 
                              "application/json", "Connection: keep-alive", ME310::TOUT_5SEC);
  
  if (rc == ME310::RETURN_VALID) {
    DEBUG_SERIAL.println("-> 2nd SND Command Accepted & JSON Sent. Waiting for Secure Server...");
    catchHttpRingAndReceive(0, 15000); 
  } else {
    DEBUG_SERIAL.println("-> [ERROR] 2nd HTTPS SND Command Failed.");
  }

  // ---------------------------------------------------------
  // [STEP 5] Cleanup and Reset Profile SSL Configuration
  // ---------------------------------------------------------
