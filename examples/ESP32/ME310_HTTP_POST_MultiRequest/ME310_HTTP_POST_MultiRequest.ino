...
void setup() {
  ...
  DEBUG_SERIAL.println("======================================");
  DEBUG_SERIAL.println(" ME310G1 HTTP POST Request Test       ");
  DEBUG_SERIAL.println("======================================");
  ...
  // ---------------------------------------------------------
  // [STEP 3] 1st POST Request: Raw String (Direct Call)
  // ---------------------------------------------------------
  ...
  if (rc == ME310::RETURN_VALID) {
    DEBUG_SERIAL.println("-> 1st SND Command Accepted & Data Sent. Waiting for Server...");
    catchHttpRingAndReceive(0, 15000); 
  } else {
    DEBUG_SERIAL.println("-> [ERROR] 1st HTTP SND Command Failed.");
  }

  delay(3000); // Give the modem a brief rest before the next transaction

  // ---------------------------------------------------------
  // [STEP 4] 2nd POST Request: JSON Payload (With content-type)
  // ---------------------------------------------------------
  ...
  if (rc == ME310::RETURN_VALID) {
    DEBUG_SERIAL.println("-> 2nd SND Command Accepted & JSON Sent. Waiting for Server...");
    catchHttpRingAndReceive(0, 15000); 
  } else {
    DEBUG_SERIAL.println("-> [ERROR] 2nd HTTP SND Command Failed.");
  }
    ...
    if (ringIndex >= 0) {
      ...
      if (newLineIndex > ringIndex) {
        ...
        if (dataSize > 0) {
          ...
          // Delegate communication and buffering 100% to the library
          myME310.debugMode(false); 
          ME310::return_t recv_rc = myME310.receive_http_data(prof_id, dataSize, ME310::TOUT_9SEC);

          // Extract data directly from the library's built-in _payloadData raw buffer
